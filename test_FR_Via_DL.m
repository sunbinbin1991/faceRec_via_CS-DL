...
% this program need identify the result of FR via dictionary learning
% at the first,the result in Yale database, the highest recognition rate
% could be 98%, however when I make the same test the result only
% 93%, here I will test ORL/Yale_B to test whether this throught useful!
% written by Xiao Li,BBS,
% date:20160107
clear; clc; close all;


%param setting=============================================================
K = 2;                  %the sparsity level used in the dictionary training
...
BlockSize = 8;          %the size of the block you extracted from images: BlockSize*BlockSize
imgRow = 112;           %the row dimension of the face size
imgCol = 92;            %the column dimension of each face
faceLib = 'ORL';        %the face label used to train and test
nPerson = 40;           %the number of the people in the lab
nFacesPerPerson = 5;    %the number of pictures to read for each person
flag_pre = 0;
...
% ColNum = floor(imgRow/BlockSize)*floor(imgCol/BlockSize); %the number of columns of each faceused for testing

[TrainFace] =ReadFaces( BlockSize, faceLib, nPerson, ...
    nFacesPerPerson, flag_pre, flag_train, ColNum);

[TestFace] =ReadFaces( BlockSize, faceLib, nPerson, ...
    nFacesPerPerson, flag_pre, flag_test, ColNum);
